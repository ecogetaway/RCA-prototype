...
func (p *Project) Migrate(m *Migrator) error {
	...
	if err = m.AddColumnIfNotExists("project", "client_id", "text"); err != nil {
		return err
	}
...
func (db *DB) GetProjects() ([]*Project, error) {
	rows, err := db.db.Query("SELECT id, name, client_id, prometheus, settings FROM project")
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	for rows.Next() {
		var p Project
		if err = rows.Scan(&p.Id, &p.Name, &clientId, &prometheus, &settings); err != nil {
			return nil, err
		}
		if clientId.Valid {
...
func (db *DB) GetProject(id ProjectId) (*Project, error) {
	...
	var settings sql.NullString
	var clientId sql.NullString
	err := db.db.QueryRow("SELECT name, client_id, prometheus, settings FROM project WHERE id = $1", id).Scan(&p.Name, &clientId, &prometheus, &settings)
	if clientId.Valid {
		p.ClientId = clientId.String
	}
	if err != nil {
...
func (db *DB) SaveProject(p *Project) error {
	...
	if p.Id == "" {
		...
		_, err := db.db.Exec("INSERT INTO project (id, name, client_id) VALUES ($1, $2, $3)", p.Id, p.Name, p.ClientId)
	...
	if _, err := db.db.Exec("UPDATE project SET name = $1, client_id = $2 WHERE id = $3", p.Name, p.ClientId, p.Id); err != nil {
		return err
	}
	return nil
}
